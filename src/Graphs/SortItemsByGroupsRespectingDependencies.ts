...
const sortItems = (n: number, m: number, group: number[], beforeItems: number[][]): number[] => {
  const initializeGraph = (n:number) => {
    let graph = [];
    for (let i = 0; i < n; i++) {
  ...
  const initializeGraphSet = (num:number) => {
    let graph = [];
    for (let i = 0; i < num; i++) {
      graph.push(new Set());
    }
  ...
  const topSort = (graph:any, inDegrees:number[]) => {
    let result = [];
    let queue = [];
  ...
  for (let item of itemOrder) group2Item[group[item]].push(item);

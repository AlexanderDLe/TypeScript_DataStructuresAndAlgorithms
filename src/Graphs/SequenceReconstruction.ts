...
const sequenceReconstructionRef = (org: number[], seqs: number[][]): boolean => {
  const pairs: any = {};
  const indexes: any = {};
  ...
  for (let j = 0; j < seqs.length; j++) {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      if (indexes[curr] === null) return false;
      ...
      if (i > 0 && indexes[prev] >= indexes[curr]) return false;
  ...
  for (let i = 0; i < org.length; i++) {
    if (!pairs[`${org[i - 1]}_${org[i]}`]) return false;
  }

  return true;
}

const sequenceReconstruction = (nums: number[], sequences: number[][]): boolean => {
  const indexes: any = {};
  const pairs: any = {};

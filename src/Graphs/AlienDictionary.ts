...
*/
const alienDictionaryRef = (words:string[]): string => {
  const inDegrees:any = {};
  const parents:any = {};
  ...
  for (let i = 0; i < words.length - 1; i++) {
    ...
    if (next.length < curr.length && curr.substring(0, next.length) === next) return '';
    ...
    while (C < curr.length && N < next.length) {
      ...
      if (sourceMap.has(child)) sourceMap.delete(child);
  ...
  const queue: string[] = Array.from(sourceMap);
  ...
  while (queue.length) {
    ...
    if (visited.has(letter)) return '';
    visited.add(letter);
    
    let childrenLetters = parents[letter];
    if (!childrenLetters) continue;
...
const alienDictionary = (words:string[]): string => {
  const inDegrees:any = {};
  const parents:any = {};
  ...
  const isPrefix = (curr:string,next:string):boolean => {
    if (curr.length > next.length && curr.substring(0, next.length) === next) return true;
    return false;
  }
  ...
  const queue:string[] = Array.from(sourceSet);

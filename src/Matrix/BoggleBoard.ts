...
const boggleBoard = (board: string[][], words:string[]): string[] => {
  ...
  const search = (row:number, col:number, substr: string, currTrie: any) => {
    if (row < 0 || row >= rows || col < 0 || col >= cols) return;
    if (pathMatrix[row][col] > 0) return;

    let char = board[row][col];
    if (!currTrie[char]) return;

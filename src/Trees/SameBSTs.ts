...
type Node = BST | null;
const sameBSTs = (array1:number[], array2:number[]): boolean => {
    const sameBSTs = (start1:number, start2:number, minVal:number, maxVal:number): boolean => {
        if (array1[start1] !== array2[start2]) return false;
    ...
    const getFirstSmallerIndex = (array: number[], start:number, minVal:number) => {
        for (let i = start + 1; i < array.length; i++) {
            let curr = array[i];
            let root = array[start];
            if (curr < root && curr >= minVal) return i;
        }
        return -1;
    }
    const getFirstBiggerIndex = (array:number[], start:number, maxVal:number) => {
        for (let i = start + 1; i < array.length; i++) {
            let curr = array[i];
            let root = array[start];
            if (curr >= root && curr < maxVal) return i;
        }
        return -1;
    }

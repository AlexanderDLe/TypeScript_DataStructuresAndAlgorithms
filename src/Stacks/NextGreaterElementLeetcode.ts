...
const nextGreaterElement = (nums1: number[], nums2: number[]): number[] => {
  ...
  for (let i = nums2.length - 1; i >= 0; i--) {
      ...
      if (!stack.length) valueToNextGreater[num] = -1;
      else valueToNextGreater[num] = stack[stack.length - 1];
      stack.push(num);
  }
  const result = []
...
export default () => {
  PrintArray(nextGreaterElement([4,1,2], [1,3,4,2]));
};

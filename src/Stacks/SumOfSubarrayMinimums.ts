...
const subarrayMinsRef = (A:number[]): number => {
  ...
  for (let i = 0; i < A.length; i++) {
    ...
    while (Pstack.length && Pstack[Pstack.length - 1].val > currVal) Pstack.pop();
...
const subarrayMins = (A:number[]): number => {
  const PLE: number[] = [];
  const NLE: number[] = [];
  const Pstack: any[] = [];
  const Nstack: any[] = [];
  ...
  for (let i = 0; i < A.length; i++) {
    ...
    while (Pstack.length && Pstack[Pstack.length - 1].val > val) Pstack.pop();
    PLE[i] = Pstack.length ? i - Pstack[Pstack.length - 1].index : i + 1;

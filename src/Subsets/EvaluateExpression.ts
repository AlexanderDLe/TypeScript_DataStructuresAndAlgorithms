...
const evaluateExpressionRef = (input: string): number[] => {
    let result: number[] = [];
        ...
        if (isNaN(parseInt(char, 10))) {
            ...
            for (let l = 0; l < left.length; l++) {
                for (let r = 0; r < right.length; r++) {
                    ...
                    if (char === '+') result.push(part1 + part2);
                    else if (char === '-') result.push(part1 - part2);
                    else if (char === '*') result.push(part1 * part2);
                }
            }
        }
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        for (let L = 0; L < left.length; L++) {
            for (let R = 0; R < right.length; R++) {
                ...
                if (char === '+') result.push(Lval + Rval)
                else if (char === '-') result.push(Lval - Rval)
                else if (char === '*') result.push(Lval * Rval);
            }
        }
    }

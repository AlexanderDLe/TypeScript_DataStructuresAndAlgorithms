...
const suggestedProductsTrie = (products:string[], searchWord:string): string[][] => {
  ...
  const buildTrie = () => {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      for (let char of product) {
        node = (node[char] = node[char] || {indexes: []});
        if (node.indexes.length < 3) node.indexes.push(i);
      }
    }
    return trie;
  }
...
const suggestedProductsBSearch = (products:string[], searchWord:string): string[][] => {
  products = products.sort();
  ...
  for (let i = 0; i < searchWord.length; i++) {
    ...
    for (let product of products) {
      if (product[i] === searchWord[i]) {
        next.push(product);
        result[i].length < 3 && result[i].push(product);
      }
    }
...
export default () => {
  PrintMatrix(suggestedProductsBSearch(["mobile","mouse","moneypot","monitor","mousepad"], "mouse"))
  // PrintMatrix(suggestedProductsBSearch(["bags","baggage","banner","box","cloths"], "bags"))
  // PrintMatrix(suggestedProductsBSearch(["havana"], "havana"))
  // PrintMatrix(suggestedProducts(["havana"], "tatiana"))
  // PrintMatrix(suggestedProducts(

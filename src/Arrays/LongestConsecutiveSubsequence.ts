...
const longestConsecutiveSubsequenceDiscovery = (nums: number[]): number => {
  let map: any = {};
  for (let num of nums) map[num] = 0;
...
const longestConsecutiveSubsequenceA = (nums: number[]): number => {
  const set = new Set();
  for (let num of nums) {
    set.add(num);
  }
...
const longestConsecutiveSubsequence = (nums: number[]): number => {
  const set = new Set();

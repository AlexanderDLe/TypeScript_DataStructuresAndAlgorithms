...
const quickselect = (a:number[], k: number): number => {
    let target = k - 1;
    
    const sortAroundPivot = (start:number, end:number): number => {
        let P = start;
        let L = start + 1;
        let R = end;
        ...
        if (R === target) return a[R];
        if (R > target) return sortAroundPivot(start, R - 1);
        if (R < target) return sortAroundPivot(R + 1, end);
    }

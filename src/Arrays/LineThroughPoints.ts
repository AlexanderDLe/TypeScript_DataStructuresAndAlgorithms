...
const lineThroughPoints = (points: number[][]): number => {
  const createHashTableKey = (rise:number, run:number) => {
    return rise.toString() + ':' + run.toString();
  }

  const getGreatestCommonDenominator = (num1:number, num2:number) => {
    let a = num1;
    let b = num2;
    while (true) {
      if (a === 0) return b;
  ...
  const getSlopeOfLineBetweenPoints = (p1:number[], p2:number[]):number[] => {
    const [p1x, p1y] = p1;
    const [p2x, p2y] = p2;
    let slope = [1, 0];
  ...
  for (let i1 = 0; i1 < points.length; i1++) {
    ...
    const currMaxPoints = Object.values(slopes).reduce((a:any, b:any) => Math.max(a, b), 0);

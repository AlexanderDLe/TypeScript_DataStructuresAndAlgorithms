...
const taskSchedulerA = (tasks: string[], n: number): number => {
  const buildHeap = () => {
    const heap = new MaxPriorityQueue({priority: (x:number) => x})
    const map: any = {};
    for (let task of tasks) {
      map[task] = (map[task] || 0) + 1;
    }  
  ...
  let queue: (number | null)[] = new Array(n).fill(null);
  let index = 0;
  ...
  while (tasksIncompleted > 0) {
    ...
    if (qItem !== null) heap.enqueue(qItem);
    console.log(queue);
    index++;
  }
...
const taskScheduler = (tasks: string[], n: number): number => {
  const buildHeap = () => {
    let heap = new MaxPriorityQueue({priority: (x:number) => x});
    let map: any = {};
    for (let task of tasks) map[task] = (map[task] || 0) + 1;
    for (let key in map) {
      heap.enqueue(map[key]);
  ...
  const queue: (number | null)[] = new Array(n).fill(null);

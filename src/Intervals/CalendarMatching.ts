...
const calendarMatching = (
  ...
  calendar2: string[][], 
  bounds2: string[], 
  meetingDuration: number): string[][] => {
  const toMins = (time: string): number => {
    let [hour, minutes] = time.split(':');
    return (Number(hour) * 60) + Number(minutes);
  }

  const getTimeDiff = (timeA: string, timeB: string): number => {
  ...
  const mergeCalendars = () => {
    ...
    let sorted = mergedCalendars.sort((a: any, b: any) => {
  ...
  const trimCalenderByBounds = (bounds: string[], schedule: string[][]) => {
    ...
    while (schedule.length) {
      ...
      if (startBound > scheduledStart) schedule[0][0] = bounds[0];
      if (endBound < scheduledEnd) schedule[schedule.length - 1][1] = bounds[1];
    
      break;
    }
  }
    
  const findAvailability = (bounds: string[], schedule: string[][]): string[][] => {
    const result: string[][] = []
  ...
  if (!mergedCalendar.length) return [mergedBounds];
...
export default () => {
  ...
  PrintMatrix(calendarMatching(calendar1, bounds1, calendar2, bounds2, meetingDuration));
};

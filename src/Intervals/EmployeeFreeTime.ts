...
class EmployeeInterval {
    ...
    constructor(interval:Interval, employeeIndex:number, intervalIndex:number) {
...
const employeeFreeTimeRef = (schedule: Interval[][]): Interval[] => {
    const sortPattern = (a:EmployeeInterval, b:EmployeeInterval) => {
    ...
    if (schedule === null || sLen === 0) return result;
...
// Use typescript type instead of class instances
type employeeInterval = {
    interval: Interval,
    employeeIndex: number,
    intervalIndex: number
}
const employeeFreeTime = (schedule: Interval[][]): Interval[] => {
    ...
    for (let i = 0; i < len; i++) {
        let startInterval: employeeInterval = {
    ...
    while (minHeap.length) {
        ...
        if (qTop.intervalIndex + 1 < employeeSchedule.length) {
            let nextInterval: employeeInterval = {

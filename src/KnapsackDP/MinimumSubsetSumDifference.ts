...
const minimumSubsetSumDifferenceTopDown = (nums:number[]): number => {
    let totalSum = nums.reduce((acc, curr) => acc + curr);
    const DP: any = {};

    const DFS = (index:number, currSum:number, outsideSum:number):number => {
        DP[index] = DP[index] || [];
...
const minimumSubsetSumDifferenceBotUp = (nums:number[]): number => {
    let totalSum = nums.reduce((acc, curr) => acc + curr);
    ...
    for (let colSum = 1; colSum < DP[0].length; colSum++) {
        if (nums[0] === colSum) DP[0][colSum] = 1;
    }

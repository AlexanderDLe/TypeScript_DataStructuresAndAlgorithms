...
const rearrangeStringKDistanceApart = (str:string, k:number): string => {
    ...
    while (maxHeap.length) {
        ...
        if (queue.length === k) {
            ...
            if (qItem.frequency > 0) maxHeap.push(qItem);
        }
    }
...
const rearrangeStringKDistanceApartSlidingWindow = (str:string, k:number): string => {
    const maxSortPattern = (a:any,b:any) => {
        return a.frequency - b.frequency;
    }
    let map: any = {};
    ...
    while (maxHeap.length) {
        ...
        while (tempStorage.length) maxHeap.push(tempStorage.pop());

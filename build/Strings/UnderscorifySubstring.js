...
const underscorifySubstringDOESNTWORK2 = (string, substring) => {
    const matchArr = new Array(string.length).fill(0);
    const validSubstring = (index) => {
        return substring === string.substring(index, index + substring.length);
    };
    const foundSubstringBehind = (index) => {
        if (index === 0)
            return false;
    ...
    (0, Utilities_1.PrintArray)(matchArr);
    let result = '';
    for (let i = 0; i < string.length; i++) {
        let char = string[i];
        let match = matchArr[i];
        if (match === 1) {
            if (!foundSubstringBehind(i))
                result += '_';
        }
        if (match === 2 && i === 0)
            result += '_';
        result += char;
        if (match === 2) {
            if (matchArr[i + 1] === 0 || i === string.length - 1)
                result += '_';
        }
    }
    return result;
...
const underscorifySubstring = (string, substring) => {
    ...
    const buildIntervals = () => {
        const validSubstr = (index) => {
            return substring === string.substring(index, index + substring.length);
        };
        for (let i = 0; i < string.length; i++) {
            if (validSubstr(i))
                intervals.push([i, i + substring.length]);
        }
    };

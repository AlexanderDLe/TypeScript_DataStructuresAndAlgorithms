...
const equalSubsetSumPartitionTopDown2 = (nums) => {
    let totalSum = nums.reduce((acc, curr) => acc + curr);
    if (totalSum % 2 > 0)
        return false;
    let target = totalSum;
    ...
    const recurse = (index, currSum) => {
        ...
        let withTake = recurse(index + 1, currSum + num);
        DP[index][currSum + num] = withTake || withoutTake;
        return DP[index][currSum + num];
    };

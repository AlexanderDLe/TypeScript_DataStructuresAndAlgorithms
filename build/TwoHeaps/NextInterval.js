...
const nextInterval = (intervals) => {
    const maxStartSort = (a, b) => { return a.start - b.start; };
    const maxEndSort = (a, b) => { return a.end - b.end; };
...
const nextIntervalQuadratic = (intervals) => {
    const minStartFirstPattern = (a, b) => {
    ...
    for (let i = 0; i < intervals.length; i++) {
        ...
        for (let j = 0; j < intervals.length; j++) {
            ...
            if (i === j)
                continue;
            if (currI[1] <= currJ[0])
                minHeap.push(new Interval(j, currJ));
        }
        if (minHeap.length) {
